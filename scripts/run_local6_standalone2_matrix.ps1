...
$schedulePath = Join-Path $out 'local6_standalone2_schedule.csv'
$manifestPath = Join-Path $out 'local6_standalone2_manifest.csv'
$matrixLog = Join-Path $logs 'local6_standalone2_matrix_controller.log'
New-Item -ItemType Directory -Force -Path $logs | Out-Null

if (-not (Test-Path $schedulePath)) {
    & $python scripts/make_local6_standalone2_schedule.py --output $schedulePath --repeats $Repeats --seed $Seed
    if ($LASTEXITCODE -ne 0) { throw 'Could not create local6/standalone2 randomization schedule' }
} else {
    ...
    if ($measuredBlocks -ne $Repeats -or [int]$existing[0].seed -ne $Seed) {
        throw 'Existing local6/standalone2 schedule does not match requested repeats/seed'
    }
}
...
foreach ($cell in $rows) {
    ...
    $runId = "v4_${prefix}$('{0:d2}' -f [int]$cell.block)_$($cell.workload)_$($cell.scenario)"
